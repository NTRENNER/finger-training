...
// every earlier rep for that grip, taking the grip's baseline — and the
// capacity floor that bounds how far a prescription may fall — with it.
describe('a long history plus one new-basis session', () => {
  ...
  test('the grip keeps its baseline', () => {
    const baselines = buildGripBaselines(mixed, buildThreeExpPriors(mixed));
  ...
  test('the capacity floor survives the session that used to erase it', () => {
    const floor = demonstratedCapacityKg(mixed, 'L', 'Micro', 90, null);
    expect(floor).toBeGreaterThan(0);
    expect(floor).toBe(demonstratedCapacityKg(history, 'L', 'Micro', 90, null));
  });
});
