// Tests for src/hooks/useCapacityHistoryByGrip.js — the per-grip capacity
...
import { useCapacityHistoryByGrip } from "../useCapacityHistoryByGrip.js";
...
describe("useCapacityHistoryByGrip", () => {
  ...
  test("cached output matches a direct uncached computation", () => {
    ...
    const baselineScore = computeBalancedCurveScore(gripBaselines.Crusher.amps);
    const expectedPct = Math.round((computeBalancedCurveScore(amps) / baselineScore - 1) * 100);
    const d3Row = out.pctRows.find(r => r.date === D3);
    expect(d3Row).toBeDefined();
    expect(d3Row.Crusher_pct).toBe(expectedPct);
